...
const getSurroundingTiles = (newPosition: Point): { tiles: Point[]; paddingTiles: Point[] } => {
    ...
    radiusTiles().forEach(({ x, y }) => {
        ...
        if (offsetX >= 0 && offsetX < MAP_DIMENSIONS.width && offsetY >= 0 && offsetY < MAP_DIMENSIONS.height) {
