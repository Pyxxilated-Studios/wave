...
extern "C" {
    ...
    #[wasm_bindgen(
        typescript_type = "{ top: Tile[][], bottom: Tile[][], left: Tile[][], right: Tile[][] }"
    )]
    pub type Padding;
...
impl MapGenerator {
    ...
    fn generate_rooms(&mut self, map: &mut Vec<Vec<Tile>>) {
        ...
        let (w, h) = (
            self.rand_between(crate::MIN_ROOM_SIZE, crate::MAX_ROOM_SIZE),
            self.rand_between(crate::MIN_ROOM_SIZE, crate::MAX_ROOM_SIZE),
        );
        ...
        (1..crate::MAX_ROOMS).for_each(|_| {
            let w = self.rand_between(crate::MIN_ROOM_SIZE, crate::MAX_ROOM_SIZE);
            let h = self.rand_between(crate::MIN_ROOM_SIZE, crate::MAX_ROOM_SIZE);
    ...
    pub fn generate(mut self) -> Map {
        ...
        JsValue::from_serde(&map).unwrap().into()
    }
}
...
///
/// # Panics
///   If the generated padding tiles fail to be cast to the correct JSValue
#[wasm_bindgen]
pub fn generate_padding_tiles() -> Padding {
    let mut rng = StdRng::from_entropy();

    // we need to add padding tiles so the player cannot see past the edge of the map

    let (top, bottom) = (0..crate::MAP_PADDING_DISTANCE)
        .map(|i| {
            (0..crate::TILE_WIDTH)
                .map(|j| {
                    (
                        Tile::new(Point::new(j, -i - 1), false, 0, rng.gen_range(1, 4)),
                        Tile::new(
                            Point::new(j, i + crate::TILE_WIDTH),
                            false,
                            0,
                            rng.gen_range(1, 4),
                        ),
    ...
    let (left, right) = (0..(crate::TILE_WIDTH + crate::MAP_PADDING_DISTANCE))
        .map(|i| {
            (0..crate::MAP_PADDING_DISTANCE)
                .map(|j| {
                    (
                        Tile::new(
                            Point::new(
                                j - crate::MAP_PADDING_DISTANCE,
                                i - crate::MAP_PADDING_DISTANCE,
                            ),
                            false,
                            0,
                            rng.gen_range(1, 4),
                        ...
                        Tile::new(
                            Point::new(j + crate::TILE_WIDTH, i - crate::MAP_PADDING_DISTANCE),
                            false,
                            0,
                            rng.gen_range(1, 4),
                        ),
    ...
    JsValue::from_serde(&PaddingTiles {
        top,
        bottom,
        left,
        right,
